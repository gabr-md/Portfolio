...
Com isso é possível mapear se aclasse de cada variável é numérica, string ou fator. Essa diferença entre as classes das varáveis pode interferir na análise descritiva e na modelagem.

Antes de prosseguir para a análise descritiva seria necessário fazer uma análise de dados faltantes (missing), NA e qualquer dado fora do comum, como é dito na descrição da base de dados que não há dados faltantes não mapeados não foi dado tanto foco para essa.
...
Serão analisadas primeiramente as variáveis de UNIFED, FONE, INSTRU e CARTAO.
...
Como a base de dados é de uma época em que telefones fixos eram mais utilizados, era esperado que a variável FONE apresentasse uma diferença maior na distribuição de bons e maus clientes entre as classes, entretando, vimos que a proporção de clientes bons e maus é semelhante, ficando próximo dos 80% de bons pagadores.
...
Para fazer a criação do modelo é feito uma divisão entre banco treino e teste, tendo o banco treino 70% da base de dados e o teste 30%. Essa divisão é feita mantendo a mesma proporção de maus clientes nos dois bancos.
...
No resultado do modelo há algumas variáveis que não foram significativas, pois o resultado do P-Valor está abaixo de 0,05 (nível de significância), visto isso, elas serão retiradas e uma nova modelagem sem essas variáveis será feita.
...
Como o objetivo da livraria é dar crédito sendo justa e não focando exclusivamente no lucro, não haverá colocar impedimentos na política de crédito para clientes com restritivo ou que compram livros de autoajuda, apesar da inadimplência maior nesses perfis. Para controlar possíveis perdas, o  e as taxas aplicadas irão variar de acordo com cada perfil, as taxas utilizadas são 2,5%, 5% e 7,5%.

Serão financiados compras de livros que fiquem na faixa de 100 à 1000 reais com planos que variam de 2 a 6 vezes, aplicando um limite que dependerá do tipo de livro comprado e da presença de restritivos ou não. Abaixo há a regra que será aplicada independente do ponto de corte a ser escolhido:
...
Analisando os resultados da escoragem o ponto de corte escolhido foi o de 602. O ponto de score 516, apesar de ter a maior podução, resulta em 10% de inadimplência. Já o ponto de score de 734, que tem a menor taxa de inadimplência, resulta em uma produção baixa quando comparada com os outros pontos.

Visando alta aprovação, baixa inadimplência e uma produção com volume, será utilizado o ponto de corte de 602 pontos.

## Conclusão

O modelo logístico aplicado aos dados apresentados conseguiu uma boa performance, ou seja, consegue dividir bem os clientes que são bons e maus pagadores. Com ajuda do modelo e um conhecimento prévio da operação e de crédito foi possível chegar em uma política de crédito interessante, que varia a taxa e o limite aplicado para cada cliente de acordo com perfil de risco, mantendo a taxa de aprovação alta e uma baixa inadimplência.
